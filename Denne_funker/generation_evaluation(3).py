...
def my_sample(model, gen_data_dir, sample_batch_size=25, obs=(3, 32, 32), sample_op=sample_op):
    ...
    for label_str in my_bidict:
        label_idx = my_bidict[label_str]
        label_tensor = torch.full((sample_batch_size,), label_idx, dtype=torch.long).to(device)

        print(f"[Generating] Class '{label_str}' (index {label_idx})...")

        sample_t = sample(model, sample_batch_size, obs, sample_op, label=label_tensor)
        sample_t = rescaling_inv(sample_t)

        save_images(sample_t, gen_data_dir, label=label_str)
