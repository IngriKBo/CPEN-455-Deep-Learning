...
def train_or_test(model, data_loader, optimizer, loss_op, device, args, epoch, mode = 'training'):
    ...
    for batch_idx, item in enumerate(tqdm(data_loader)):
        model_input, label = item  #get label
        model_input = model_input.to(device)

        # convert list of strings to tensor of indices
        # filter out unknown labels
        filtered_items = [(i, l) for i, l in enumerate(label) if l in my_bidict]
        if not filtered_items:
            continue  # skip this batch if no valid labels

        indices, valid_labels = zip(*filtered_items)
        model_input = model_input[list(indices)].to(device)  # filter inputs

        label_indices = torch.tensor([my_bidict[l] for l in valid_labels], dtype=torch.long).to(device)

        model_output = model(model_input, label=label_indices)

        # label = label.to(device)


        # model_output = model(model_input, label=label)
    ...
    # if a batch is skipped we want to avoid dividing by 0        
    if args.en_wandb and loss_tracker.count > 0:
       wandb.log({mode + "-Average-BPD" : loss_tracker.get_mean()})
       wandb.log({mode + "-epoch": epoch})
    elif loss_tracker.count == 0:
       print(f"[Warning] No valid data processed in {mode} mode for epoch {epoch}.")
...
if __name__ == '__main__':
    ...
    model = PixelCNN(nr_resnet=args.nr_resnet, nr_filters=args.nr_filters, 
                input_channels=input_channels, nr_logistic_mix=args.nr_logistic_mix, num_classes=4)
    ...
    for epoch in tqdm(range(args.max_epochs)):
        ...
        if epoch % args.sampling_interval == 0:
            ...
            label_tensor = torch.full((args.sample_batch_size,), 0, dtype=torch.long).to(device)
