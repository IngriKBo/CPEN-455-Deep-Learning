...
class PixelCNNLayer_up(nn.Module):
    ...
    def forward(self, u, ul, cond=None):  #ADDING CONDITION
        u_list, ul_list = [], []

        for i in range(self.nr_resnet):
            u  = self.u_stream[i](u, cond=cond) # add condition
            ul = self.ul_stream[i](ul, a=u, cond=cond) # add condition
            u_list  += [u]
            ul_list += [ul]
...
class PixelCNNLayer_down(nn.Module):
    ...
    def forward(self, u, ul, u_list, ul_list, cond=None):
        for i in range(self.nr_resnet):
            u  = self.u_stream[i](u, a=u_list.pop(), cond=cond) # condition
            ul = self.ul_stream[i](ul, a=torch.cat((u, ul_list.pop()), 1), cond=cond) # condition
...
class PixelCNN(nn.Module):
    def __init__(self, nr_resnet=5, nr_filters=80, nr_logistic_mix=10,
                    resnet_nonlinearity='concat_elu', input_channels=3, num_classes=4): # added num_classes=4
        ...
        self.label_embedding = nn.Embedding(num_classes, nr_filters)  # added label conditioning
    ...
    def forward(self, x, label=None, sample=False):
        ...
        for i in range(3):
            # resnet block
            u, ul = self.down_layers[i](u, ul, u_list, ul_list, cond=cond)
